...
This seems rather innocent, but consider the normal use of containers today:
  . A developer wants to deploy an application using a mongo-db container
  . The developer pulls a mongo-db container image from Docker Hub and builds the app
  . The developer deploys the container(s)
  . That was easy...

Containerized applications are easy to deploy, after all that is one of the key benefits.  But how do you know what vulnerabilities or trojans ship along with the selected container image (ie: what's inside)?  Who will maintain and patch the image going forward?  How can you protrect your environment from weak design practices?
