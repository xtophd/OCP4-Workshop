...
In this solution, we are going to create a volume (ie: filesystem) which provides sufficient read/write permissions for the default (and more secure) UID.  The type of volume we will be using is called an *emptyDir*.
...
For the purpose of this exercise, *emptyDir* suits our needs so let’s proceed to deploy and configure the pod to use an emptyDir as a volume mounted under /var/www/html.
