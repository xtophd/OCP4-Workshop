...
Once the **hello-app-1-????** is in a 'Running' state, you can proceed with verification that the POD is operating correctly.
...
[NOTE]
====
_Alternate command(s)s to verify POD service_
----
oc get services

curl http://{ip_address}}:8080
----
Use the 'Cluster-IP' address from output of `oc get services` to build the URL for *curl*.
====
...
[NOTE]
====
_Alternate manaul steps to rsh to POD_
----
oc get pods
...
[NOTE]
====
_Alternate command(s) to rsh to POD_
----
oc get pods
...
[NOTE]
====
_Alternate command(s) to validate application_
----    
curl http://helloworld.cloud.example.com
----
====
