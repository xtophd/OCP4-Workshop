...
The current working project is denoted in a pull down on the *Navigation Bar*.  For the purposes of these exercises, we are focused on `Applications`, `Builds` and `Storage`.  Other attributes of a *Project* will be covered in later units.
...
If the image name was entered correctly, when you press `Return` (or hit the `Search` icon) additional parameters will appear in order to complete this phase.
