...
As noted, the current working project is denoted in a pull down on the *Navigation Bar*.  This is important to monitor to ensure your actions are being applied to the correct project.
...
IMPORTANT:  Again, it is critical to understand that this is not a recommended solution to deploying an application into production.  This solution reduces your overall security by changes project attributes.  This solution is also non-persistent.  If the application is stopped and restarted, or fails and restarts ... the changes you introduced to the image by manually copying index.html to the image will not be restored.
