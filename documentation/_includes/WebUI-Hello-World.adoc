...
As noted in solution #1, there is a security issue with the default UID and filesystem permissions in base containers directory /var/www/html.  Solution #1 attacked the problems by changing the UID of the running process.

In this solution, we are going to create a volume (ie: filesystem) which provides read/write permissions for the default (and more secure) UID.
