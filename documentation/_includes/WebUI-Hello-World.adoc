...
____
select `Terminal` from *Pods Overview*
____
...
Before you selected `Create` you had an opportunity to inspect the file.  The only change to the project's security attributes is _openshift.io/sascc.uid-range:1001/10000_.  This is identical to the changes made during the CLI Hello World exercises using the `oc edit project` command line.
...
select `Terminal` from *Pod Details*
____
