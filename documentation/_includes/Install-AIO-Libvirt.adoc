...
If enabled in master-config.yml, a VNC server is configured on the bastion host.

You could then use an ssh tunnel to setup up a port on the virt host which will connect to the bastion vm.
...
Don't forget to set the deisplay quality to "High" in the connection properties.
