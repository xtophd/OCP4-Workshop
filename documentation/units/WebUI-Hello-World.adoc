...
One of th eoptions in the pod details is to connect to the active pods terminal.  Proceed to click on `Terminal`.

====
image::ocp-webui-hw1-12-pod-terminal.png[Pod Terminal]
====

Now that you have connected to the active container, have a look around.  These are identical exercises to those performed in the CLI version of hellow-world.  If you are comfortable with the activites, can can skip to **Making an authentic "Hello, World!"**.
